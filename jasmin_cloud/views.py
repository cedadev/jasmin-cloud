...
from .provider import base as provider_base
    ...
    return response.Response(
        {
            ...
            "links": {
                "authenticate": request.build_absolute_uri(
                    reverse("jasmin_cloud:authenticate")
                ),
    ...
    return response.Response(
        {
            ...
            "links": {
                "tenancies": request.build_absolute_uri(
                    reverse("jasmin_cloud:tenancies")
                )
            },
        }
    )
    ...
    * ``ram``: The amount of RAM (in MB).
    ...
    with request.auth.scoped_session(tenant) as session:
        serializer = serializers.SizeSerializer(
            session.sizes(), many=True, context={"request": request, "tenant": tenant}
        )
    return response.Response(serializer.data)
...
def size_details(request, tenant, size):
    ...
    * ``ram``: The amount of RAM (in MB).
    ...
    with request.auth.scoped_session(tenant) as session:
        serializer = serializers.SizeSerializer(
            session.find_size(size), context={"request": request, "tenant": tenant}
        )
    return response.Response(serializer.data)
    ...
    if request.method == "PUT":
        with request.auth.scoped_session(tenant) as session:
            ...
            input_serializer = serializers.UpdateClusterSerializer(
                data=request.data, context=dict(session=session, cluster=cluster)
            )
            input_serializer.is_valid(raise_exception=True)
        ...
        output_serializer = serializers.ClusterSerializer(
            updated, context={"request": request, "tenant": tenant}
        )
        return response.Response(output_serializer.data)
    elif request.method == "DELETE":
        with request.auth.scoped_session(tenant) as session:
